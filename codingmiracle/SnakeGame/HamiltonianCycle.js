class HamiltonianCycle {
        ...
        for (let n of degree1Nodes) {
            ...
            for (let m of degree1Nodes) {
                if (dist(n.x, n.y, m.x, m.y) === 1) {
                    if (Math.floor(n.x / 2) === Math.floor(m.x / 2) && Math.floor(n.y / 2) === Math.floor(m.y / 2)) {
    ...
    createSpanningTree() {
        ...
        let spanningTree = [];
        let randomNode = stNodes[Math.floor(Math.random() * stNodes.length)];
...
class HPath {
    ...
    getSnakeTailPositionAfterFollowingPath(snake) {
        if (this.pathLength - snake.addCount < snake.snakeElements.length) {
            return snake.snakeElements[Math.max(0, this.pathLength - snake.addCount)];
        }
        ...
        return this.nodesInPath[tailMoved - snake.snakeElements.length];
    }
    ...
    clone() {
        var clone = new HPath(this.nodesInPath[0], this.finishNode);
